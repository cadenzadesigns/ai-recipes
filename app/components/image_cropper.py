...
class StreamlitImageCropper:
    ...
    def crop_single_image_canvas(
        ...
        # Description
        st.info(
            "🖱️ Click and drag to draw rectangles around recipe images. You can crop multiple regions from the same page."
        )
        ...
        if st.session_state.crop_regions[image_key]:
            st.success(
                f"✅ {len(st.session_state.crop_regions[image_key])} image(s) selected"
            )
        ...
        st.caption(
            f"📏 Original: {original_image.width}×{original_image.height}px | Thumbnail: {thumbnail.width}×{thumbnail.height}px"
        )
        ...
        with col1:
            ...
            with col_a:
                is_main = st.radio(
                    ...
                    index=(
                        0 if len(st.session_state.crop_regions[image_key]) == 0 else 1
                    ),
                )
        ...
        with col2:
            ...
            if st.button("✅ Save This Crop", key=f"{image_key}_save", type="primary"):
                ...
                if crop_valid:
                    ...
                    scaled_coords["x1"] = max(
                        0, min(scaled_coords["x1"], original_image.width)
                    )
                    scaled_coords["y1"] = max(
                        0, min(scaled_coords["y1"], original_image.height)
                    )
                    scaled_coords["x2"] = max(
                        0, min(scaled_coords["x2"], original_image.width)
                    )
                    scaled_coords["y2"] = max(
                        0, min(scaled_coords["y2"], original_image.height)
                    )
                    ...
                    crop_data = {
                        ...
                        or (
                            "Main recipe image"
                            if is_main == "Main recipe image"
                            else "Recipe step image"
                        ),
                    }
                    ...
                    st.error(
                        "❌ Please draw a crop region first - click and drag on the image to select an area"
                    )
            ...
            if st.session_state.crop_regions[image_key]:
                for i, crop in enumerate(st.session_state.crop_regions[image_key]):
                    ...
                    with col_b:
                        if st.button(
                            "🗑️", key=f"{image_key}_remove_{i}", help="Remove this crop"
                        ):
                            st.session_state.crop_regions[image_key].pop(i)
                            # Reset crop index if we removed crops
                            if st.session_state[crop_key] > len(
                                st.session_state.crop_regions[image_key]
                            ):
                                st.session_state[crop_key] = len(
                                    st.session_state.crop_regions[image_key]
                                )
                            st.rerun()
            else:
            ...
            # Clear all button
            if st.session_state.crop_regions[image_key] and st.button(
                "🗑️ Clear All", key=f"{image_key}_clear_all"
            ):
                st.session_state.crop_regions[image_key] = []
                st.session_state[crop_key] = 0
                st.rerun()
