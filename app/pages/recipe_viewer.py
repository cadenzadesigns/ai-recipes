    ...
    if recipe_data.get("images"):
        ...
        if main_images:
            ...
            if img_path.exists():
                ...
                st.image(str(img_path), use_container_width=True, caption=img.get("description"))
                ...
                st.markdown(f'<img src="data:{mime_type};base64,{encoded_string}" width="1" height="1" style="position: absolute; left: -9999px;" alt="{recipe_title}" />', unsafe_allow_html=True)
    ...
    if recipe_data.get("description"):
        ...
        for paragraph in paragraphs:
            if paragraph.strip():  # Only display non-empty paragraphs
                st.markdown(f'<p class="recipe-description">{paragraph.strip()}</p>', unsafe_allow_html=True)
